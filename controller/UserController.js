...
exports.getAllUsers = async (req, res) => {
    try {
        const users = await User.find({ deleted_at: null }).select('-roles');
...
exports.deleteUser = async (req, res) => {
    try {
        // Find the user by ID and set `deleted_at` to the current date
        const deletedUser = await User.findByIdAndUpdate(
            req.params.id,
            { deleted_at: new Date() }, // Soft delete: mark the deleted_at field
        ...
        res.status(200).json({ success: true, message: "User soft deleted successfully!", error: 0 });
    } catch (error) {
        res.status(500).json({ error: 1, message: "Internal Server Error", details: error.message });
    }
};
